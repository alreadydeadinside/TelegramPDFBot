...
def handle_stop(message):
    bot.reply_to(message, "Please enter the name of your file without .pdf)")
...
def handle_start(message):
    ...
    bot.send_message(message.chat.id, "Hi there, send me images then write /quit to get a pdf")
...
def handle_rename(message):
    try:
        ...
        images = []
        if length == 0:
            bot.reply_to(message, "Sorry, you didnt provide any files, try again")
            return
